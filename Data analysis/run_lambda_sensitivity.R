...
eval_lambdas = evaluateCompetitors(mf_lambdas, sel_viruses, 
                                   rownames(scores_lambdas_diff[[1]]))

# Compute AUC and recall (Fig S10)
eval_lambdas_diff = evaluateCompetitors(scores_lambdas_diff, sel_viruses, 
                                        rownames(scores_lambdas_diff[[1]]))
...
pdf(paste0(wd, "AUC_lambdas.pdf"), width = 8)
boxplot(eval_lambdas$auc, pch=20, col=colors, ylab="AUC", cex.lab=1.5, 
        cex.axis=1.5)
dev.off()

# Supplementary Fig S10
pdf(paste0(wd, "AUC_lambdas_diff.pdf"), width = 8)
